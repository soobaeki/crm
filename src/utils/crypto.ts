import { decryptGCM } from "@/lib/crypto/crypto";
 ...
 * - 복호화 실패 시 원본 반환
 *
 * @param payload 암호화된 문자열 (encryptGCM 결과)
 * @returns 복호화된 문자열 또는 원본
 */
export function safeDecryptGCM(payload: string | null): string | null {
  if (!payload) return null;

  try {
    return decryptGCM(payload);
  } catch {
    return payload; // 이미 복호화되어 있거나 잘못된 값
  }
}
